...
static character_t *io_nearest_visible_trainer()
{
  ...
  c = (character_t **) malloc(world.cur_map->get_num_trainers() * sizeof (*c));
...
void io_display()
{
  ...
  for (y = 0; y < MAP_Y; y++) {
    for (x = 0; x < MAP_X; x++) {
      if (world.cur_map->cmap[y][x]) {
        mvaddch(y + 1, x, world.cur_map->cmap[y][x]->get_symbol());
      } else {
        switch (world.cur_map->mp[y][x]) {
        case ter_boulder:
  ...
  * not otherwise used.                                                 */
  ...
  mvprintw(22, 1, "%d known %s.", world.cur_map->get_num_trainers(),
           world.cur_map->get_num_trainers() > 1 ? "trainers" : "trainer");
...
uint32_t io_teleport_pc(pair_t dest)
{
  ...
  do {
    ...
    dest[dim_y] = rand_range(1, MAP_Y - 2);
           ...
           move_cost[char_pc][world.cur_map->mp[dest[dim_y]]
                                                [dest[dim_x]]] ==
             DIJKSTRA_PATH_MAX                                            ||
...
static void io_list_trainers()
{
  ...
  c = (character **) malloc(world.cur_map->get_num_trainers() * sizeof (*c));
...
void io_battle(character *aggressor, character *defender)
{
  ...
  if (npc->get_ctype() == char_hiker || npc->get_ctype() == char_rival) {
...
uint32_t move_pc_dir(uint32_t input, pair_t dest)
{
  ...
  switch (input) {
    ...
    case 9:
      ...
      break;
    case '>':
      if (world.cur_map->mp[world.pc.pos[dim_y]][world.pc.pos[dim_x]] ==
          ter_mart) {
        io_pokemart();
      }
      if (world.cur_map->mp[world.pc.pos[dim_y]][world.pc.pos[dim_x]] ==
  ...
  //TODO leave the map...
  if (world.cur_map->mp[dest[dim_y]][dest[dim_x]] == ter_gate || dest[dim_y] == 0) {
     
    if (dest[dim_y] == world.cur_map->e && dest[dim_x] > 75) {          // east gate
      world.cur_idx[dim_x]++;
      // move pc coords here...
      dest[dim_x] = 1;
    } else if (dest[dim_y] == world.cur_map->w && dest[dim_x] < 5) {    // west gate
      world.cur_idx[dim_x]--;
      // move pc coords here...
      dest[dim_x] = 78;
    } else if (dest[dim_x] == world.cur_map->n && dest[dim_y] > 18) {   // north gate
      world.cur_idx[dim_y]--;
      // move pc coords here...
      dest[dim_y] = 19;
    } else if (dest[dim_x] == world.cur_map->s && dest[dim_y] < 3) {    // south gate
      world.cur_idx[dim_y]++;
      // move pc coords here...
      dest[dim_y] = 1;
    }
  ...
  if (world.cur_map->cmap[dest[dim_y]][dest[dim_x]]) {
    if (world.cur_map->cmap[dest[dim_y]][dest[dim_x]]->npc &&
        world.cur_map->cmap[dest[dim_y]][dest[dim_x]]->get_defeated()) {
  ...
  if (move_cost[char_pc][world.cur_map->mp[dest[dim_y]][dest[dim_x]]] ==
      DIJKSTRA_PATH_MAX) {
    return 1;
  }
...
void io_get_fly() {
  ...
  int x = 0, y = 0;
  //getting input
  mvprintw(0, 0, "Enter x, y coord here: ");
  mvscanw(0, 23, "%d, %d", &x, &y);
  cbreak();
  noecho();
