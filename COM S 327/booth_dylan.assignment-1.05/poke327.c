...
/*
  Forward declaring function call as it is near bottom of file,
  with the other code wrote for this assignment. Keeping work together
  and easy to find.
*/
void attempt_battle(character_t *c);
...
static void move_hiker_func(character_t *c, pair_t dest)
{
  ...
  if (world.cur_map->cmap[dest[dim_y]][dest[dim_x]] == &world.pc) {
    attempt_battle(c);
  }
...
/*
  Only used for pc movement
*/
int is_possible_move(pair_t d) {
  
  if (d[dim_x] <= 0 || d[dim_x] >= 79 || d[dim_y] <= 0 || d[dim_y] >= 20) {
        return 1;
    }

  if (    world.cur_map->map[d[dim_y]][d[dim_x]] != ter_grass     &&   // tall grass
          world.cur_map->map[d[dim_y]][d[dim_x]] != ter_clearing  &&   // short grass
          world.cur_map->map[d[dim_y]][d[dim_x]] != ter_path      &&   // path
          world.cur_map->map[d[dim_y]][d[dim_x]] != ter_mart      &&   // pokemart
          world.cur_map->map[d[dim_y]][d[dim_x]] != ter_center) {      // pokecenter
      return 1;
  }

  if (    world.cur_map->map[d[dim_y]][d[dim_x]] == ter_water     ||
          world.cur_map->map[d[dim_y]][d[dim_x]] == ter_mountain  ||
          world.cur_map->map[d[dim_y]][d[dim_x]] == ter_boulder   ||
          world.cur_map->map[d[dim_y]][d[dim_x]] == ter_tree      ||
          world.cur_map->map[d[dim_y]][d[dim_x]] == ter_forest) {
      return 1;
  }
...
static void move_pc_func(character_t *c, pair_t dest, int ch)
{
  switch (ch) {
      case 'y':
      case 55:                            // 7
  ...
  if (world.cur_map->cmap[dest[dim_y]][dest[dim_x]] && is_possible_move(dest) == 0) {
    if (world.cur_map->cmap[dest[dim_y]][dest[dim_x]]->npc->defeated == 0) {
      attempt_battle(world.cur_map->cmap[dest[dim_y]][dest[dim_x]]);
    } else {
      // dont move
      dest[dim_x] = c->pos[dim_x];
      dest[dim_y] = c->pos[dim_y];
    }
  } else if (is_possible_move(dest) != 0) {
    // dont move
    dest[dim_x] = c->pos[dim_x];
    dest[dim_y] = c->pos[dim_y];
  }
...
void new_hiker()
{
  ...
  enqueue(&world.cur_map->trainers, c);
...
static int new_map()
{
  ...
  initQueue(&world.cur_map->trainers);
...
/*
  commented out to prevent -wall -werror warnigs.
*/
...
/*
  Controls the map printing using ncurses to refresh and 
  display information.
*/
  ...
  for (i = 0; i < MAP_Y; i++) {
    for (j = 0; j < MAP_X; j++) {
      if (world.cur_map->cmap[i][j]) {
        if (world.cur_map->cmap[i][j] == &world.pc) {
          attron(color_mapping[8] | A_BOLD);
          mvaddch(i+1, j, world.cur_map->cmap[i][j]->symbol);
          attroff(color_mapping[8] | A_BOLD);
        } else {
          attron(color_mapping[8]);
          mvaddch(i+1, j, world.cur_map->cmap[i][j]->symbol);
          attroff(color_mapping[8]);
        }
      } else {
        switch (world.cur_map->map[i][j]) {
          case ter_boulder:
            mvaddch(i+1, j, BOULDER_SYMBOL);
            break;
          case ter_mountain:
            attron(color_mapping[2]);
            mvaddch(i+1, j, MOUNTAIN_SYMBOL);
            attroff(color_mapping[2]);
            break;
          case ter_tree:
            mvaddch(i+1, j, TREE_SYMBOL);
            break;
          case ter_forest:
            attron(color_mapping[4]);
            mvaddch(i+1, j, FOREST_SYMBOL);
            attroff(color_mapping[4]);
            break;
          case ter_path:
            attron(color_mapping[5]);
            mvaddch(i+1, j, PATH_SYMBOL);
            attroff(color_mapping[5]);
            break;
          case ter_gate:
            attron(color_mapping[5]);
            mvaddch(i+1, j, GATE_SYMBOL);
            attroff(color_mapping[5]);
            break;
          case ter_mart:
            attron(color_mapping[7]);
            mvaddch(i+1, j, POKEMART_SYMBOL);
            attroff(color_mapping[7]);
            break;
          case ter_center:
            attron(color_mapping[8]);
            mvaddch(i+1, j, POKEMON_CENTER_SYMBOL);
            attroff(color_mapping[8]);
            break;
          case ter_grass:
            attron(color_mapping[0]);
            mvaddch(i+1, j, TALL_GRASS_SYMBOL);
            attroff(color_mapping[0]);
            break;
          case ter_clearing:
            attron(color_mapping[3]);
            mvaddch(i+1, j, SHORT_GRASS_SYMBOL);
            attroff(color_mapping[3]);
            break;
          case ter_water:
            attron(color_mapping[1]);
            mvaddch(i+1, j, WATER_SYMBOL);
            attroff(color_mapping[1]);
            break;
          default:
            mvaddch(i+1, j, ERROR_SYMBOL);
            break;
        }
      }
    }
...
/*
  Used for entering and exiting menus for pokemart and pokecenter.
  Checks if position is valid for command and which building to enter.
*/
void attempt_enter(character_t *c) {

  int ch;

  if (world.cur_map->map[c->pos[dim_y]][c->pos[dim_x]] == ter_mart) {           // pokemart
    clear();
    box(stdscr, 0, 0);
    mvprintw(3, 5, "PokeMart!");
    mvprintw(6, 5, "Time to buy some pokeballs and potions");
    mvprintw(10, 5, "Enter '<' to close and return to map!");
    refresh();
  } else if (world.cur_map->map[c->pos[dim_y]][c->pos[dim_x]] == ter_center) {  // pokecenter
    clear();
    box(stdscr, 0, 0);
    mvprintw(3, 5, "PokeCenter");
    mvprintw(6, 5, "Time to let those pokemon sleep or whatever happens here");
    mvprintw(10, 5, "Enter '<' to close and return to map!");
    refresh();
  } else {
    return;
...
/*
  Menu to list all trainers on map, relative location in each direction,
  and whether or not the trainer has been defeated.
*/
void list_trainers() {
  ...
  while (1) {
    box(stdscr, 0, 0);
    mvprintw(2, 5, "Trainers:\t\t\t\tDefeated");

    int max_list_num = 18; //num of trainers to be visible
    int num_trainers = world.cur_map->trainers.size;
    int num_to_display = (num_trainers - list_start) < max_list_num ? (num_trainers - list_start) : max_list_num;
    struct queue temp;
    initQueue(&temp);

    for (int i = 0; i < num_to_display; i++) {
      character_t *c = (character_t *) dequeue(&world.cur_map->trainers);
      int rel_x, rel_y;

      rel_x = world.pc.pos[dim_x] - c->pos[dim_x];
      rel_y = world.pc.pos[dim_y] - c->pos[dim_y];

      if (rel_x < 0 && rel_y < 0) {           // north west
        mvprintw(i + 3, 5, "%c, %3d north and %3d west\t\t%s", c->symbol, abs(rel_y), abs(rel_x), def[c->npc->defeated]);
      } else if (rel_x > 0 && rel_y < 0) {    // north east
        mvprintw(i + 3, 5, "%c, %3d north and %3d east\t\t%s", c->symbol, abs(rel_y), rel_x, def[c->npc->defeated]);
      } else if (rel_x == 0 && rel_y < 0) {   // north
        mvprintw(i + 3, 5, "%c, %3d north\t\t\t%s", c->symbol, abs(rel_y), def[c->npc->defeated]);
      } else if (rel_x < 0 && rel_y == 0) {   // west
        mvprintw(i + 3, 5, "%c, %3d west\t\t\t%s", c->symbol, abs(rel_x), def[c->npc->defeated]);
      } else if (rel_x > 0 && rel_y == 0) {   // east
        mvprintw(i + 3, 5, "%c, %3d east\t\t\t%s", c->symbol, rel_x, def[c->npc->defeated]);
      } else if (rel_x < 0 && rel_y > 0) {    // south west
        mvprintw(i + 3, 5, "%c, %3d south and %3d west\t\t%s", c->symbol, rel_y, abs(rel_x), def[c->npc->defeated]);
      } else if (rel_x > 0 && rel_y > 0) {    // south east
        mvprintw(i + 3, 5, "%c, %3d south and %3d east\t\t%s", c->symbol, rel_y, rel_x, def[c->npc->defeated]);
      } else {                                // south
        mvprintw(i + 3, 5, "%c, %3d south\t\t\t%s", c->symbol, rel_y, def[c->npc->defeated]);
      }
    ...
    while (!empty(&temp)) {
      enqueue(&world.cur_map->trainers, dequeue(&temp));
    }

    wattron(stdscr, A_STANDOUT);
    mvprintw(selected - list_start + 3, 2, ">");
    ...
    switch (choice) {
      case KEY_UP:
        ...
        break;
      case KEY_DOWN:
        if (selected < num_trainers - 1 && selected < list_start + max_list_num - 1) {
          selected++;
          if (selected >= list_start + max_list_num) {
            list_start++;
          }
        }
        break;
...
/*
  Menu for a battle to occur. Currently only commands are escape and defeat/win battles.
  The defeat is to change path traversal of r/h to move like explorers.
*/
void attempt_battle(character_t *c) {
  int choice;
  ...
  clear();
  box(stdscr, 0, 0);
  mvprintw(3, 5, "entered battle");
  mvprintw(5, 5, "enter 4 to attack, this just changes traversal if h/r");
  mvprintw(6, 5, "enter esc to leave fight.");
  ...
  while (1) {
    ...
    switch (choice) {
      case 52:
        mvprintw(10, 5, "you attacked and defeated the trainer %c", c->symbol);
        c->npc->defeated = 1;
        break;
      case 27:
        clear();
  ...
  while (1) {
    ...
    if (c == &world.pc) {
      ...
      if (ch == 'Q' || ch == 'q') { // quit game
        break;
      } else if (ch == '>') {       // enter a mart/center
        attempt_enter(c);
      } else if (ch == 't') {       // display list of trainers
        list_trainers();
      } else if (ch == 27) {        // here as a possible fix to bug i had, otherwise does nothing
        ...
        mvprintw(23, 0, "world pos: (%d, %d)  pc pos: (%d, %d)", world.cur_idx[dim_x], world.cur_idx[dim_y], 
...
/*
  Sets the color pairs for the array of color_pairs at beginning of file.
*/
void init_colors() {
    init_pair(1,    COLOR_GREEN,       COLOR_BLACK);   // tall grass
    init_pair(2,    COLOR_CYAN,        COLOR_BLACK);   // water
    init_pair(3,    COLOR_MAGENTA,     COLOR_BLACK);   // mountain
    init_pair(4,    COLOR_GREEN,       COLOR_BLACK);   // short grass
    init_pair(5,    COLOR_WHITE,       COLOR_BLACK);   // trees
    init_pair(6,    COLOR_YELLOW,      COLOR_BLACK);   // path
    init_pair(7,    COLOR_RED,         COLOR_BLACK);   // pokemart
    init_pair(8,    COLOR_RED,         COLOR_BLACK);   // pokecenter
    init_pair(9,    COLOR_RED,         COLOR_BLACK);   // pc/npc
}

/*
  Handles seg fault issues upon resizing of terminal window.
*/
...
int main(int argc, char *argv[])
{
  ...
  // until next comment is ncurses setup
  initscr();
  start_color();
  resize_term(24, 80);
  init_colors();
