...
void io_battle(character *aggressor, character *defender)
{
  ...
  io_display();
  mvprintw(0, 0, "Trainer %c has a pokemon %s against your pokemon %s.", n->symbol, n->poke.at(0)->name.c_str(), world.pc.poke.at(0)->name.c_str());
  refresh();
  getch();
...
void io_display_pokemon_info(int ch) {
    ...
    for (int i = 0; i < (int) cur->moves.size(); i++) {
      mvprintw(9+i, 5, "%d. %s, power: %d, accuracy: %d", i+1, moves_list.at(cur->moves.at(i).move_id).identifier.c_str(), moves_list.at(cur->moves.at(i).move_id).power, moves_list.at(cur->moves.at(i).move_id).accuracy);
    }
...
void io_pokemon() {
  ...
  for (i = 0; i < world.pc.poke.size(); i++) {
    ...
    mvprintw(7 + i, 5, "%d. %s hp:%d attack:%d defense:%d special attack:%d special defense:%d speed:%d", i+1, cur->name.c_str(), cur->hp, cur->attack, cur->defense, cur->s_attack, cur->s_defense, cur->speed);
  }
