...
std::string gym_types[4] = {
    "Scheaffer's Gym",
    "Elnaz Bashir's Gym",
    "Ehsan Ghafourian's Gym",
    "Asif Siddique's Gym",
};

std::string gym_trainer_names[4] = {
    "Scheaffer",
    "Elnaz Bashir",
    "Ehsan Ghafourian",
    "Asif Siddique",
...
static int place_gym(map *m) {
  pair_t p;
  gym *new_gym = new gym;
  npc *trainer = new npc;

  // setting gym type, whos is it
  int type = rand() % 4;
  new_gym->gym_name = gym_types[type];
  new_gym->trainer_name = gym_trainer_names[type];

  // making trainer for gym
  trainer->ctype = char_other;
  trainer->defeated = false;
  trainer->symbol = 't';

  int man_dist = world.cur_idx[dim_x] + world.cur_idx[dim_y] - 400;
  // trainer->poke.push_back(spawnPokemon(man_dist));

  while (trainer->poke.size() < 3) {
    trainer->poke.push_back(spawnPokemon(man_dist));
  }

  trainer->cur_pokemon = trainer->poke.at(0);

  new_gym->trainer = trainer;

  // finding gym location
  find_building_location(m, p);

  mapxy(p[dim_x]    , p[dim_y]    ) = ter_gym;
  mapxy(p[dim_x] + 1, p[dim_y]    ) = ter_gym;
  mapxy(p[dim_x]    , p[dim_y] + 1) = ter_gym;
  mapxy(p[dim_x] + 1, p[dim_y] + 1) = ter_gym;
  world.cur_map->gmap[p[dim_y]    ][p[dim_x]    ] = new_gym;
  world.cur_map->gmap[p[dim_y] + 1][p[dim_x]    ] = new_gym;
  world.cur_map->gmap[p[dim_y]    ][p[dim_x] + 1] = new_gym;
  world.cur_map->gmap[p[dim_y] + 1][p[dim_x] + 1] = new_gym;
...
void new_hiker()
{
  ...
  int man_dist = world.cur_idx[dim_x] + world.cur_idx[dim_y];

  c->poke.push_back(spawnPokemon(man_dist));
  while (rand() % 10 > 4 && c->poke.size() <= 6) {
    c->poke.push_back(spawnPokemon(man_dist));
  }

  c->cur_pokemon = c->poke.at(0);
...
void init_pc()
{
  ...
  world.pc.item_bag.max_size = 100;

  world.pc.poke.push_back(io_choose_pokemon());
