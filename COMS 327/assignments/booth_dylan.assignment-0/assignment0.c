...
int main(int argc, char *argv[])
{
  ...
  // The Sobel Kernel for the edges on the x plane
  int kX[3][3] = {
    {-1, 0, 1},
    {-2, 0, 2},
    {-1, 0, 1}
  };
  // The Sobel Kernel for the edges on the y plane
  int kY[3][3] = {
    {-1, -2, -1},
    { 0,  0,  0}, 
    { 1,  2,  1}
  ...
  int n = kernelSize;       // Kernel size
  int kC = ceil(n / 2);     // Kernel center
    ...
    for (int c = 0; c < imageSize; c++) {
      ...
      for (int j = 0; j < kernelSize; j++) {
        for (int i = 0; i < kernelSize; i++) {
          accumX += kX[j][i] * image[r + (j - kC)][c + (i - kC)];
          accumY += kY[j][i] * image[r + (j - kC)][c + (i - kC)];
        }
      }
