...
void verticalPath(struct terrain *terr, int startX, int endX) {
    ...
    while (currX != endX || currY != ROWS - 1) {
        
        if (currX < endX) {
            currX++;
        } else if (currX > endX) {
            currX--;
        } else if (currY < ROWS -1) {
            currY++;
        } else if (currY > ROWS -1) {
            currY--;
        }
...
void horizontalPath(struct terrain *terr, int startY, int endY) {
    ...
    while (currX != COLUMNS - 1 || currY != endY) {
        
        if (currY < endY) {
            currY++;
        } else if (currY > endY) {
            currY--;
        } else if (currX < COLUMNS - 1) {
            currX++;
        } else if (currX > COLUMNS - 1) {
            currX--;
        }
...
int isAdjacent(struct terrain *terr, int x, int y) {
    ...
    if (x >= 0 && x < COLUMNS && y >= 0 && y < ROWS) {
        if (terr->map[x][y] == terrType[5] ||
            ...
            (x < COLUMNS - 1 && terr->map[x + 1][y] == terrType[5]) ||
            (y > 0 && terr->map[x][y - 1] == terrType[5]) ||
            (y < ROWS - 1 && terr->map[x][y + 1] == terrType[5])) {
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        printf("(%d, %d) Enter command: ", world.currX, world.currY);
        scanf(" %s", &choice);
