...
void map::set_num_trainers(int n) {
  num_trainers = n;
}

int32_t map::increment_num_trainers() {
  return num_trainers++;
}

int32_t map::get_num_trainers() {
  return num_trainers;
}
// End of map class functions
...
static terrain_type_t border_type(map *m, int32_t x, int32_t y)
{
  ...
  for (q = miny; q < maxy; q++) {
    for (p = minx; p < maxx; p++) {
      if (q != y || p != x) {
        if (m->mp[q][p] == ter_mountain ||
            m->mp[q][p] == ter_boulder) {
          r++;
        } else if (m->mp[q][p] == ter_forest ||
                   m->mp[q][p] == ter_tree) {
          t++;
        }
      }
...
static int place_boulders(map *m)
{
  ...
  for (i = 0; i < MIN_BOULDERS || rand() % 100 < BOULDER_PROB; i++) {
    y = rand() % (MAP_Y - 2) + 1;
    x = rand() % (MAP_X - 2) + 1;
    if (m->mp[y][x] != ter_forest &&
        m->mp[y][x] != ter_path   &&
        m->mp[y][x] != ter_gate) {
      m->mp[y][x] = ter_boulder;
    }
  }
...
void new_hiker()
{
  ...
  world.cur_map->cmap[pos[dim_y]][pos[dim_x]] = (character *) malloc(sizeof (*c));

  c = new character(char_hiker, move_hiker);

  c->set_pos(pos);
  c->set_dir(dir);
  c->set_symbol(HIKER_SYMBOL);
  c->set_next_turn(0);
  c->set_seq_num(world.char_seq_num++);

  heap_insert(world.cur_map->get_turn(), c);
}
...
void new_rival()
{
  ...
  c = new character(char_rival, move_rival);

  c->set_pos(pos);
  c->set_dir(dir);
  c->set_symbol(RIVAL_SYMBOL);
  c->set_next_turn(0);
  c->set_seq_num(world.char_seq_num++);
...
void new_swimmer()
{
  ...
  do {
    rand_pos(pos);
  } while (world.cur_map->mp[pos[dim_y]][pos[dim_x]] != ter_water ||
           world.cur_map->cmap[pos[dim_y]][pos[dim_x]]);
  ...
  c = new character(char_swimmer, move_swim);

  c->set_pos(pos);
  c->set_dir(dir);
  c->set_symbol(SWIMMER_SYMBOL);
  c->set_next_turn(0);
  c->set_seq_num(world.char_seq_num++);
...
void new_char_other()
{
  ...
  switch (rand() % 4) {
  case 0:
    c = new character(char_other, move_pace);
    c->set_symbol(PACER_SYMBOL);
    break;
  case 1:
    c = new character(char_other, move_wander);
    c->set_symbol(WANDERER_SYMBOL);
    break;
  case 2:
    c = new character(char_other, move_sentry);
    c->set_symbol(SENTRY_SYMBOL);
    break;
  case 3:
    c = new character(char_other, move_explore);
    c->set_symbol(EXPLORER_SYMBOL);
    break;
  }
...
{
  world.cur_map->set_num_trainers(3);
  ...
  } while (world.cur_map->increment_num_trainers() < MIN_TRAINERS ||
...
void init_pc()
{
  int x, y;
  pair_t pos;
  character c = world.pc;

  do {
    x = rand() % (MAP_X - 2) + 1;
    y = rand() % (MAP_Y - 2) + 1;
  } while (world.cur_map->mp[y][x] != ter_path);

  pos[dim_x] = x;
  pos[dim_y] = y;

  // world.pc = new character(PC_SYMBOL);
  world.pc.set_pos(pos);
  world.pc.set_symbol(PC_SYMBOL);
  world.pc.set_next_turn(0);
  world.pc.set_seq_num(world.char_seq_num++);

  world.cur_map->cmap[y][x] = &world.pc;

  heap_insert(world.cur_map->get_turn(), &world.pc);
}
...
void place_pc()
{
  ...
  int16_t x = world.pc.pos[dim_x], y = world.pc.pos[dim_y];
  ...
  if ((c = (character *) heap_peek_min(world.cur_map->get_turn()))) {
    world.pc.set_next_turn(c->get_next_turn());
  } else {
    world.pc.set_next_turn(0);
  }
}
...
int new_map(int teleport)
{
  ...
  heap_init(world.cur_map->get_turn(), cmp_char_turns, delete_character);
  ...
  if (teleport) {
    do {
      ...
      world.pc.pos[dim_y] = rand_range(1, MAP_Y - 2);
             ...
             (move_cost[char_pc][world.cur_map->mp[world.pc.pos[dim_y]]
                                                   [world.pc.pos[dim_x]]] ==
              DIJKSTRA_PATH_MAX)                                           ||
...
void delete_world()
{
  ...
  heap_delete(world.cur_map->get_turn());
...
void game_loop()
{
  ...
  while (!world.quit) {
    c = (character *) heap_remove_min(world.cur_map->get_turn());

    move_func[c->npc ? c->get_mtype() : move_pc](c, d);

    world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x]] = NULL;
    world.cur_map->cmap[d[dim_y]][d[dim_x]] = c;

    if (typeid(c) == typeid(character)) {
      pathfind(world.cur_map);
    }

    c->set_next_turn(c->get_next_turn() + move_cost[c->npc ? c->get_ctype() : char_pc]
                                [world.cur_map->mp[d[dim_y]][d[dim_x]]]);
