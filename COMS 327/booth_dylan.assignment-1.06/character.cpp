...
#define is_adjacent(pos, ter)                                     \
  ((world.cur_map->mp[pos[dim_y] - 1][pos[dim_x] - 1] == ter) || \
   (world.cur_map->mp[pos[dim_y] - 1][pos[dim_x]    ] == ter) || \
   (world.cur_map->mp[pos[dim_y] - 1][pos[dim_x] + 1] == ter) || \
   (world.cur_map->mp[pos[dim_y]    ][pos[dim_x] - 1] == ter) || \
   (world.cur_map->mp[pos[dim_y]    ][pos[dim_x] + 1] == ter) || \
   (world.cur_map->mp[pos[dim_y] + 1][pos[dim_x] - 1] == ter) || \
   (world.cur_map->mp[pos[dim_y] + 1][pos[dim_x]    ] == ter) || \
   (world.cur_map->mp[pos[dim_y] + 1][pos[dim_x] + 1] == ter))
...
/*
  All character class functions till next major comment
*/
  character::character() { }
  ...
  character::character(character_type_t ct, movement_type_t mt) {
    character::ctype = ct;
    character::mtype = mt;
    character::npc = true;
  ...
  void character::set_pos(pair_t new_pos) {
    character::pos[dim_x] = new_pos[dim_x];
    character::pos[dim_y] = new_pos[dim_y];
  }
  ...
  void character::set_symbol(char sym) {
    character::symbol = sym;
  }  

  int character::get_next_turn() const {
    return character::next_turn;
  }

  void character::set_next_turn(int next_turn) {
    character::next_turn = next_turn;
  };

  int character::get_seq_num() const {
    return seq_num;
  }

  void character::set_seq_num(int seq_num) {
    character::seq_num = seq_num;
  }

  character_type_t character::get_ctype() const {
    return ctype;
  }

  movement_type_t character::get_mtype() const {
    return mtype;
  }

  void character::set_mtype(movement_type_t m) {
    mtype = m;
  }

  bool character::get_defeated() const {
    return defeated;
  }

  void character::set_defeated() {
    defeated = true;
  }

  void character::set_dir(pair_t d) {
    character::dir[0] = d[0];
    character::dir[1] = d[1];
  }
/*
  End of character class functions
*/
...
static void move_pacer_func(character *c, pair_t dest)
{
  ...
  t = world.cur_map->mp[c->pos[dim_y] + c->dir[dim_y]]
                        [c->pos[dim_x] + c->dir[dim_x]];
...
static void move_wanderer_func(character *c, pair_t dest)
{
  ...
  if ((world.cur_map->mp[c->pos[dim_y] + c->dir[dim_y]]
                         [c->pos[dim_x] + c->dir[dim_x]] !=
       world.cur_map->mp[c->pos[dim_y]][c->pos[dim_x]]) ||
      world.cur_map->cmap[c->pos[dim_y] + c->dir[dim_y]]
                         [c->pos[dim_x] + c->dir[dim_x]]) {
    rand_dir(c->dir);
  }
...
static void move_explorer_func(character *c, pair_t dest)
{
  ...
  if ((move_cost[char_other][world.cur_map->mp[c->pos[dim_y] +
                                                c->dir[dim_y]]
                                               [c->pos[dim_x] +
                                                c->dir[dim_x]]] ==
...
static void move_swimmer_func(character *c, pair_t dest)
{
  ...
  if (is_adjacent(world.pc.pos, ter_water) &&
    ...
    if ((m->mp[dest[dim_y] + dir[dim_y]]
               [dest[dim_x] + dir[dim_x]] == ter_water) ||
        ((m->mp[dest[dim_y] + dir[dim_y]]
    ...
    } else if ((m->mp[dest[dim_y]][dest[dim_x] + dir[dim_x]] == ter_water) ||
               ((m->mp[dest[dim_y]][dest[dim_x] + dir[dim_x]] == ter_path) &&
                is_adjacent(((pair_t){ (int16_t) (dest[dim_x] + dir[dim_x]),
                                       (int16_t) (dest[dim_y]) }), ter_water))) {
      dest[dim_x] += dir[dim_x];
    } else if ((m->mp[dest[dim_y] + dir[dim_y]][dest[dim_x]] == ter_water) ||
               ((m->mp[dest[dim_y] + dir[dim_y]][dest[dim_x]] == ter_path) &&
    ...
    dir[dim_x] = c->dir[dim_x];
    dir[dim_y] = c->dir[dim_y];
    if ((m->mp[dest[dim_y] + dir[dim_y]]
                    [dest[dim_x] + dir[dim_x]] != ter_water) ||
        !((m->mp[dest[dim_y] + dir[dim_y]]
                 [dest[dim_x] + dir[dim_x]] == ter_path) &&
          is_adjacent(((pair_t) { (int16_t) (dest[dim_x] + (int16_t) dir[dim_x]),
                                  (int16_t) (dest[dim_y] + dir[dim_y]) }), ter_water))) {
      rand_dir(dir);
    }
...
{
  return ((((character *) key)->get_next_turn() ==
           ((character *) with)->get_next_turn())  ?
          (((character *) key)->get_seq_num() -
           ((character *) with)->get_seq_num())    :
          (((character *) key)->get_next_turn() -
           ((character *) with)->get_next_turn()));
}
...
#define ter_cost(x, y, c) move_cost[c][m->mp[y][x]]
