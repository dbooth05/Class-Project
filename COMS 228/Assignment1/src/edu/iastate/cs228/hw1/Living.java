	...
	{	
		...
		for(int i = -1; i < 2; i++) {
			...
			for(int j = -1; j < 2; j++) {
				...
				try {
					if(plain.grid[row + i][column + j].who() == State.BADGER) {
							population[BADGER] += 1;
					}	else if(plain.grid[row + i][column + j].who() == State.EMPTY) {
							population[EMPTY] += 1;
					}	else if(plain.grid[row + i][column + j].who() == State.FOX) {
							population[FOX] += 1;
					}	else if(plain.grid[row + i][column + j].who() == State.GRASS) {
							population[GRASS] += 1;
					}	else if(plain.grid[row + i][column + j].who() == State.RABBIT) {
							population[RABBIT] += 1;
					}
				}	
				catch(Exception e) {
					//Only catches exceptions cause by indexOutOfBounds 
					//And does nothing
				}
			}
		}
